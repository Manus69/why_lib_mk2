...
static void _merge(Vector_X** target,
                Vector_X** lhs,
                Vector_X** rhs,
                size_t lhs_length, 
                size_t rhs_length, 
                int (*cmp)(const Vector_X*, const Vector_X*))
{
    while (true)
    {
        if (lhs_length == 0)
        {
            return (void)memcpy(target, rhs, rhs_length * sizeof(Vector_X*));
        }
        if (rhs_length == 0)
        {
            return (void)memcpy(target, lhs, lhs_length * sizeof(Vector_X*));
        }
...
static void _pass(Vector_X** source, 
                Vector_X** target, 
                size_t frame_size, 
                size_t length, 
                int (*cmp)(const Vector_X*, const Vector_X*))
...
void MergeSort_VX(Vector_X** data, size_t index, size_t length, int (*cmp)(const Vector_X*, const Vector_X*))
{
    Vector_X**   buffer;
    size_t  count;
    size_t  n_iterations;
    ...
    buffer = malloc(sizeof(Vector_X*) * length);
    ...
    if (n_iterations % 2)
        memcpy(data + index, buffer, sizeof(Vector_X*) * length);
